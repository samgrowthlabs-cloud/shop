  ...
  try {
    ...
    const runComparisonModel = /* @__PURE__ */ __name((model, attempt, structured) => env.AI.run(model, {
      ...
      gateway: {
        ...
        skipCache: false,
        cacheTtl: 2592e3,
        cacheKey: `premium-comparison-v13:${attempt}:${structured ? "schema" : "json"}:${version}`,
        collectLog: true,
        metadata: { feature: "premium-product-comparison", comparisonVersion: "v13", productSlugs: [...slugs].sort().join(",") }
      }
    }), "runComparisonModel");
...
async function analyzeProductComparison(req, env, ctx, id) {
  ...
  const version = await sha256(`comparison-v13|${comparisonAiSetting.modelId}|${comparisonAiSetting.fallbackModelId || ""}|${products.map((product) => `${product.slug}:${product.updatedAt}:${product.price}:${product.fullDescription}:${JSON.stringify(product.specifications)}`).join("|")}`);
