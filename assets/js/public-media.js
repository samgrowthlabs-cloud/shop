...
async function detailMedia(){
  ...
  mainButton.addEventListener('click',event=>openGallery(items,Number(event.currentTarget.dataset.index)||0,data.name));
  box.querySelector('.detail-thumbs')?.addEventListener('click',event=>{const button=event.target.closest('[data-index]');if(!button)return;const index=Number(button.dataset.index);if(button.classList.contains('more-images')){openGallery(items,index,data.name);return}const item=items[index],image=box.querySelector('.detail-main-image img');image.src=url(item);image.alt=item.altText||data.name||'Produto';mainButton.dataset.index=index;mainButton.classList.remove('is-magnifying');box.querySelectorAll('.detail-thumbs button').forEach(thumb=>thumb.classList.toggle('active',thumb===button))});
}
