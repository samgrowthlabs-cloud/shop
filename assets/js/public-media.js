...
async function renderProductNews(product,host){
  ...
  section.innerHTML=`${hook?`<header>${hook}</header>`:''}<div class="product-related-news-skeleton" aria-hidden="true">${Array.from({length:4},()=>'<span><i></i><b></b><em></em></span>').join('')}</div>`;
  placeProductNews(section,host);
  mediaQuery.addEventListener('change',()=>placeProductNews(section,host));
  const show=items=>{
    const cards=items.map(article=>{const image=new URL(article.coverUrl,C.API_BASE_URL).href;return `<a class="product-related-news-card" href="/noticias/${encodeURIComponent(article.slug)}"><img src="${safe(image)}" alt="${safe(article.image?.alt||article.title)}" loading="lazy" decoding="async"><strong>${safe(article.title)}</strong></a>`}).join('');
    section.classList.remove('is-loading');section.removeAttribute('aria-busy');section.innerHTML=`${hook?`<header>${hook}</header>`:''}<div>${cards}</div>`;
  };
