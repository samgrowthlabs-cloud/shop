...
import'./favicon.js?v=20260803-media-domain-38';import{signUp,signIn,signOut,recover,updatePassword,updateAccountCredentials,acceptRedirectSession,session,currentUser,apiProfile,userApi,startPresence}from'./auth-20260827-v5.js?v=20260918-current-password-1';
...
async function account(){
  ...
  const paintAvatar=id=>{const target=$(id);if(!target)return;if(avatar){target.innerHTML=`<img src="${esc(avatar)}" alt="Foto de ${esc(displayName)}" referrerpolicy="no-referrer">`;return}if(!target.querySelector("img"))target.innerHTML=id==='#profile-account-avatar'?`<b style="color:#fff!important">${esc(initials)}</b>`:`<b>${esc(initials)}</b>`};
  ...
  $('#account-password-form').onsubmit=async event=>{event.preventDefault();const form=event.currentTarget,button=form.querySelector('button'),currentPassword=$('#account-current-password').value,password=$('#account-new-password').value,confirmation=$('#account-confirm-password').value;if(password!==confirmation){securityMessage('As senhas não coincidem. Digite a mesma senha nos dois campos.');return}if(password.length<8){securityMessage('A nova senha precisa ter pelo menos 8 caracteres.');return}if(currentPassword===password){securityMessage('A nova senha precisa ser diferente da senha atual.');return}button.disabled=true;try{await updateAccountCredentials({currentPassword,password});form.reset();securityMessage('Senha alterada com sucesso.','success')}catch(error){securityMessage(error.message)}finally{button.disabled=false}};
  const deleteForm=$('#delete-account-form'),deleteConfirmation=$('#delete-account-confirmation'),deleteButton=deleteForm?.querySelector('button'),deleteDialog=$('#delete-account-dialog'),cancelDeleteButton=$('#cancel-delete-account'),confirmDeleteButton=$('#confirm-delete-account');
  if(deleteForm&&deleteConfirmation&&deleteButton&&deleteDialog&&cancelDeleteButton&&confirmDeleteButton){
    deleteConfirmation.oninput=()=>{deleteButton.disabled=deleteConfirmation.value.trim().toUpperCase()!=='EXCLUIR'};
    deleteForm.onsubmit=event=>{event.preventDefault();if(deleteConfirmation.value.trim().toUpperCase()!=='EXCLUIR')return;deleteDialog.showModal();cancelDeleteButton.focus()};
    cancelDeleteButton.onclick=()=>deleteDialog.close();
    deleteDialog.onclick=event=>{if(event.target===deleteDialog)deleteDialog.close()};
