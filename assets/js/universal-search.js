...
export async function initUniversalSearch(){
  ...
  const productIcon='<svg class="universal-section-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="m4 7 8-4 8 4-8 4-8-4Z"></path><path d="M4 7v10l8 4 8-4V7M12 11v10"></path></svg>',newsIcon='<svg class="universal-section-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M5 4h12a2 2 0 0 1 2 2v13H7a2 2 0 0 1-2-2V4Z"></path><path d="M19 8h2v9a2 2 0 0 1-2 2M8.5 8h7M8.5 11.5h7M8.5 15h4"></path></svg>';
  const allProducts=products.slice(0,8).map(state.card).join(''),allNews=news.slice(0,4).map(newsCard).join(''),allSections=[products.length?`<section class="universal-result-section"><header><div><h2>${productIcon}<span>Produtos <small>(${products.length})</small></span></h2></div>${products.length>8?'<button type="button" data-show-all="produtos">Ver todos os produtos →</button>':''}</header><div class="products universal-products">${allProducts}</div></section>`:'',news.length?`<section class="universal-result-section universal-news-section"><header><div><h2>${newsIcon}<span>Notícias <small>(${newsTotal})</small></span></h2></div>${newsTotal>4?'<button type="button" data-show-all="noticias">Ver todas as notícias →</button>':''}</header><div class="universal-news-grid">${allNews}</div></section>`:''].join('');
  ...
  listing.insertAdjacentHTML('beforebegin',`<div class="container universal-search-panels"><div data-search-panel="todos" role="tabpanel">${allSections||empty}</div><div data-search-panel="noticias" role="tabpanel">${news.length?`<section class="universal-result-section universal-news-section"><header><div><h2>${newsIcon}<span>Notícias <small>(${newsTotal})</small></span></h2></div></header><div class="universal-news-grid is-full">${news.map(newsCard).join('')}</div></section>`:`<div class="universal-empty"><strong>Nenhuma notícia encontrada para “${esc(query)}”.</strong><span>Confira os produtos ou tente outros termos.</span></div>`}</div></div>`);
