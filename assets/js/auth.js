...
export async function signUp({name,email,password}){return request(`signup?redirect_to=${encodeURIComponent(new URL('auth-callback.html',location.href).href)}`,{email,password,data:{display_name:name}})}
...
export async function recover(email){return request(`recover?redirect_to=${encodeURIComponent(new URL('redefinir-senha.html',location.href).href)}`,{email})}
