...
const publicAuthUrl=path=>new URL(`/${path}`,location.origin).href;
export async function signUp({name,email,password}){return request(`signup?redirect_to=${encodeURIComponent(publicAuthUrl('auth-callback'))}`,{email,password,data:{display_name:name}})}
