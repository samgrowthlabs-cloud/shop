...
async function request(path,body,token,method='POST'){let response;try{response=await fetch(`${C.SUPABASE_URL}/auth/v1/${path}`,{method,headers:{...headers(),...(token?{authorization:`Bearer ${token}`}:{})},body:body===undefined?undefined:JSON.stringify(body)})}catch(error){const failure=Error(authError({message:error?.message}));failure.status=0;throw failure}const data=await response.json().catch(()=>({}));if(!response.ok){const failure=Error(authError(data,response.status));failure.status=response.status;failure.code=data.code||data.error_code||data.error||'';throw failure}return data}
