...
function requestComparisonAnalysis(slugs, products, attempt = 0, runId = comparisonRunId) {
  const cached = attempt === 0 ? readComparisonCache(slugs) : null;
  if (cached) { applyAnalysisToUI(cached, products); return; }
  getComparisonAnalysis(slugs)
    .then(analysis => {
      if (!analysis?.processing) writeComparisonCache(slugs, analysis);
