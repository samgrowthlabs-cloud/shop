...
const KEY='shoplab:user-session',REFERRAL_KEY='shoplab:referral-attribution',headers=()=>({'apikey':C.SUPABASE_PUBLISHABLE_KEY,'content-type':'application/json'});let refreshPromise=null;
...
export async function updateAccountCredentials(changes={}){const current=await validSession();if(!current)throw Error('Entre novamente para alterar os dados de acesso.');const body={};if(changes.email)body.email=String(changes.email).trim();if(changes.currentPassword)body.current_password=String(changes.currentPassword);if(changes.password)body.password=String(changes.password);if(!Object.keys(body).length)throw Error('Nenhuma alteração foi informada.');return request('user',body,current.access_token,'PUT')}
...
export async function validSession(){let current=session();if(!current)return null;if(Number(current.expires_at||0)>Math.floor(Date.now()/1000)+30)return current;if(!current.refresh_token){save(null);return null}if(!refreshPromise){const refresh=async()=>{const latest=session();if(!latest)return null;if(Number(latest.expires_at||0)>Math.floor(Date.now()/1000)+30)return latest;if(!latest.refresh_token)return null;const attemptedAccessToken=latest.access_token;try{const renewed=await request('token?grant_type=refresh_token',{refresh_token:latest.refresh_token}),stored=session();if(stored?.access_token&&stored.access_token!==attemptedAccessToken&&Number(stored.expires_at||0)>=Number(renewed.expires_at||0))return stored;return save(renewed)}catch{const stored=session();return stored?.access_token!==attemptedAccessToken?stored:null}};refreshPromise=(navigator.locks?.request?navigator.locks.request('shoplab-auth-refresh',refresh):refresh()).finally(()=>{refreshPromise=null})}return refreshPromise}
export async function currentUser(){const current=await validSession();if(!current)return null;try{return await request('user',undefined,current.access_token,'GET')}catch{return null}}
...
export async function userApi(path,options={}){
  ...
  if(response.status===401&&session()?.access_token===current.access_token){save(null);}
