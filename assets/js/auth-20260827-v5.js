...
export async function validSession(){let current=session();if(!current)return null;if(Number(current.expires_at||0)>Math.floor(Date.now()/1000)+30)return current;if(!current.refresh_token){save(null);return null}if(!refreshPromise)refreshPromise=request('token?grant_type=refresh_token',{refresh_token:current.refresh_token}).then(save).catch(()=>session()).finally(()=>{refreshPromise=null});return refreshPromise}
