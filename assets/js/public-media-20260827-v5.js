...
import{session,userApi}from'./auth-20260827-v5.js?v=20260922-invalid-refresh-cleanup-2';
...
async function getProduct(slug){if(cache.has(slug))return cache.get(slug);const endpoint=C.API_BASE_URL+'/api/v1/products/'+encodeURIComponent(slug)+'?media-preview=3';const promise=fetch(endpoint,{credentials:'include',cache:'no-store'}).then(response=>response.ok?response.json():null).then(payload=>payload?.data||payload).catch(()=>null);cache.set(slug,promise);return promise}
...
const cardVideoStart=video=>Math.max(0,Number(video?.dataset.previewStart)||0);
const startCardVideo=video=>{if(!video)return;const play=()=>{const start=Math.min(cardVideoStart(video),Math.max(0,(video.duration||Infinity)-.1));try{if(video.currentTime<start||video.currentTime>=start+6)video.currentTime=start}catch{}video.muted=true;video.play().catch(()=>{})};if(video.readyState>=1)play();else video.addEventListener('loadedmetadata',play,{once:true})};
const stopCardVideo=card=>{const video=card?.querySelector('.product-video-preview');if(!video)return;video.pause();try{video.currentTime=cardVideoStart(video)}catch{}};
document.addEventListener('pointerover',event=>{if(!matchMedia('(hover:hover) and (pointer:fine)').matches)return;const card=event.target.closest?.('.product-card.has-video-preview');startCardVideo(card?.querySelector('.product-video-preview'))},{passive:true});
...
function placeProductNews(section,host){
  if(!section||!host)return;
  const description=document.querySelector('.product-description-section');
  if(matchMedia('(max-width:760px)').matches&&description){
    description.insertAdjacentElement('beforebegin',section);
    section.classList.add('is-mobile-positioned');
    return;
  }
  host.append(section);
  section.classList.remove('is-mobile-positioned');
}

function newsVisitorId(){
  try{let value=localStorage.getItem('shoplab:visitor-id');if(!value){value=crypto.randomUUID();localStorage.setItem('shoplab:visitor-id',value)}return value}catch{return''}
}
function normalizedNewsText(value){return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase('pt-BR').replace(/[^a-z0-9]+/g,' ').trim()}
function productNewsTerms(product){
  const stop=new Set(['para','com','sem','de','do','da','dos','das','em','um','uma','e','ou','por','the','edition','edicao','produto','novo','nova','branco','preto','azul','verde','gb','ram','tela']);
  const values=[product?.brand,product?.model,product?.category,product?.categoryName,product?.name,...(Array.isArray(product?.tags)?product.tags:[])].filter(Boolean),terms=[];
  for(const value of values)for(const term of normalizedNewsText(value).split(' '))if(term.length>=3&&!stop.has(term)&&!terms.includes(term))terms.push(term);
  return terms;
}
function productNewsScore(article,product){
  const title=normalizedNewsText(article.title),body=normalizedNewsText([article.subtitle,article.excerpt,article.category,...(Array.isArray(article.tags)?article.tags:[])].join(' ')),all=title+' '+body,brand=normalizedNewsText(product?.brand),category=normalizedNewsText(product?.category||product?.categoryName),terms=productNewsTerms(product);
  let score=0,matches=0;
  if(brand&&all.includes(brand)){score+=5;matches++}
  if(category&&all.includes(category)){score+=3;matches++}
  for(const term of terms){if(!all.includes(term))continue;score+=title.includes(term)?3:1;matches++}
  return{score,matches};
}
function productNewsQueries(product){
  const terms=productNewsTerms(product),brand=String(product?.brand||'').trim(),category=String(product?.category||product?.categoryName||'').trim(),distinctive=terms.find(term=>/\d/.test(term))||terms.find(term=>term.length>=6);
  return[brand,distinctive,category].map(value=>String(value||'').trim()).filter((value,index,list)=>value&&list.indexOf(value)===index).slice(0,3);
}
function productNewsHook(product){
  if(!session())return'';
  let profile={};try{profile=JSON.parse(localStorage.getItem('shoplab:account-header')||'null')||{}}catch{}
  const sessionUser=session()?.user,name=String(sessionUser?.user_metadata?.display_name||sessionUser?.user_metadata?.full_name||profile.name||'').trim().split(/\s+/)[0],subject=String(product?.brand||product?.category||'este tipo de produto').trim();
  let recent=[];try{recent=JSON.parse(localStorage.getItem('shoplab:viewed-categories')||'[]')}catch{}
  const interest=recent.find(item=>item?.name)?.name||product?.category||subject;
  return `<p class="product-related-news-hook"><span>PARA VOCÊ${name?`, ${safe(name)}`:''}</span> Conteúdos sobre ${safe(subject)} selecionados porque você acompanha ${safe(interest)} na SHOPLAB.</p>`;
}
async function fetchProductNews(path,visitor){
  const response=await fetch(`${C.API_BASE_URL}${path}`,{headers:visitor?{'x-shoplab-visitor':visitor}:{}});
  if(!response.ok)return null;
  return response.json();
}

async function renderProductNews(product,host){
  if(!product?.slug||!host||document.querySelector('.product-related-news'))return;
  const hook=productNewsHook(product),section=document.createElement('section'),mediaQuery=matchMedia('(max-width:760px)');
  section.className='product-related-news is-loading';
  section.setAttribute('aria-label','Notícias selecionadas para este produto');
  section.setAttribute('aria-busy','true');
  section.innerHTML=`${hook?`<header class="product-related-news-personalization">${hook}</header>`:''}<div class="product-related-news-skeleton" aria-hidden="true">${Array.from({length:4},()=>'<span><i></i><b></b><em></em></span>').join('')}</div>`;
  placeProductNews(section,host);
  mediaQuery.addEventListener('change',()=>placeProductNews(section,host));
  const show=items=>{
    const cards=items.map(article=>{const image=new URL(article.coverUrl,C.API_BASE_URL).href,date=article.publishedAt?new Intl.DateTimeFormat('pt-BR',{day:'2-digit',month:'short'}).format(new Date(article.publishedAt)):'';return `<a class="product-related-news-card" href="/noticias/${encodeURIComponent(article.slug)}"><img src="${safe(image)}" alt="${safe(article.image?.alt||article.title)}" loading="lazy" decoding="async"><span class="product-related-news-copy"><small>${safe(article.category||'Notícias')}</small><strong>${safe(article.title)}</strong><span>${Math.max(1,Number(article.readingTime)||1)} min${date?` · ${safe(date)}`:''}</span></span></a>`}).join('');
    section.classList.remove('is-loading');section.removeAttribute('aria-busy');section.innerHTML=`${hook?`<header class="product-related-news-personalization">${hook}</header>`:''}<div class="product-related-news-list">${cards}</div>`;
  };
  const cacheKey=`shoplab:product-news:v2:${product.slug}`;
  try{const cached=JSON.parse(sessionStorage.getItem(cacheKey)||'null');if(cached&&Date.now()-cached.savedAt<600000&&cached.items?.length){show(cached.items);return}}catch{}
  try{
    const visitor=newsVisitorId(),params=new URLSearchParams({product:product.slug,limit:'8',visitor}),queries=productNewsQueries(product),[primary,...searches]=await Promise.all([fetchProductNews(`/api/v1/news?${params}`,visitor),...queries.map(query=>fetchProductNews(`/api/v1/news?${new URLSearchParams({q:query,limit:'12',visitor})}`,visitor).catch(()=>null))]),primaryData=primary?.data||primary||{},direct=primaryData.product===product.slug,candidates=[...(primaryData.items||[])];
    searches.forEach(result=>candidates.push(...((result?.data||result)?.items||[])));
    const unique=[...new Map(candidates.map(article=>[article.id||article.slug,article])).values()],ranked=unique.map(article=>({...article,_relevance:productNewsScore(article,product)})).filter(article=>direct||article._relevance.score>=5&&article._relevance.matches>=2).sort((a,b)=>b._relevance.score-a._relevance.score||Date.parse(b.publishedAt||0)-Date.parse(a.publishedAt||0)),items=ranked.slice(0,4).map(article=>({...article,coverUrl:article.image?.url||article.video?.thumbnailUrl||''})).filter(article=>article.coverUrl);
...
function enableImageMagnifier(button,image){
  if(!button||!image||button.dataset.magnifierReady)return;
  button.dataset.magnifierReady='1';
  let magnifier=button.querySelector('.detail-image-magnifier');
  if(!magnifier){magnifier=document.createElement('span');magnifier.className='detail-image-magnifier';magnifier.setAttribute('aria-hidden','true');button.append(magnifier)}
  const sync=()=>{magnifier.style.backgroundImage=`url("${String(image.currentSrc||image.src).replaceAll('"','%22')}")`};
  sync();image.addEventListener('load',sync);
  if(!matchMedia('(hover:hover) and (pointer:fine)').matches)return;
  button.addEventListener('pointerenter',sync);
  button.addEventListener('pointermove',event=>{const stageRect=button.getBoundingClientRect(),imageRect=image.getBoundingClientRect(),x=event.clientX-stageRect.left,y=event.clientY-stageRect.top,sourceX=event.clientX-imageRect.left,sourceY=event.clientY-imageRect.top;if(sourceX<0||sourceX>imageRect.width||sourceY<0||sourceY>imageRect.height){button.classList.remove('is-magnifying');return}const lens=156,zoom=2.15,lensX=Math.max(lens/2+8,Math.min(stageRect.width-lens/2-8,x)),lensY=Math.max(lens/2+8,Math.min(stageRect.height-lens/2-8,y));button.style.setProperty('--magnifier-x',`${lensX}px`);button.style.setProperty('--magnifier-y',`${lensY}px`);magnifier.style.backgroundSize=`${imageRect.width*zoom}px ${imageRect.height*zoom}px`;magnifier.style.backgroundPosition=`${lens/2-sourceX*zoom}px ${lens/2-sourceY*zoom}px`;button.classList.add('is-magnifying')});
  button.addEventListener('pointerleave',()=>button.classList.remove('is-magnifying'));
}
async function detailMedia(){
  ...
  const slug=new URLSearchParams(location.search).get('slug'),data=slug?await getProduct(slug):null,enhance=()=>{renderPromotion(data);renderDescriptions(data);renderProductInformation(data);arrangeMobileOffer();const actionObserver=new MutationObserver(()=>{if(document.querySelector('.user-product-actions')){arrangeMobileOffer();actionObserver.disconnect()}});actionObserver.observe(document.querySelector('.page-hero .detail')||document.body,{childList:true,subtree:true});matchMedia('(max-width:760px)').addEventListener('change',arrangeMobileOffer);renderPremiumProductInsight(data)},items=(data?.media||[]).filter(item=>url(item));if(!items.length){enhance();return}const featuredVideo=items.find(item=>item.type==='video'&&item.isHover);if(featuredVideo){const imageItems=items.filter(item=>item.type==='image'),cover=imageItems.find(item=>item.isPrimary)||imageItems[0],galleryItems=[featuredVideo,...imageItems],start=Math.max(0,Number(featuredVideo.previewStartSeconds)||0),dots=galleryItems.map((item,index)=>`<span class="${index===0?'active':''}" aria-hidden="true"></span>`).join(''),thumbs=galleryItems.map((item,index)=>`<button type="button" class="${index===0?'active':''}${item.type==='video'?' video-thumb':''}" data-featured-index="${index}" aria-label="${item.type==='video'?'Ver vídeo':`Ver imagem ${index}`}">${item.type==='video'?`<span class="video-thumb-content" style="display:grid;place-items:center;gap:3px"><svg class="video-thumb-play-svg" width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="18" cy="18" r="16.5" fill="white" stroke="#26302E" stroke-width="3"/><path d="M14.5 11.8L25 18L14.5 24.2V11.8Z" fill="#26302E"/></svg><span class="video-thumb-label" style="display:block;font-size:11px;font-weight:800;line-height:1;color:#17201e;text-decoration:underline">Vídeo</span></span>`:`<img src="${safe(url(item,160))}" alt="">`}</button>`).join('');box.classList.add('has-featured-video');box.classList.toggle('has-multiple-media',galleryItems.length>1);box.innerHTML=`<div class="detail-featured-stage detail-main-image" style="border:0!important;outline:0!important;box-shadow:none!important;background:transparent!important;appearance:none!important"></div>${galleryItems.length>1?`<div class="detail-gallery-dots" aria-label="Mídia 1 de ${galleryItems.length}">${dots}</div>`:''}<div class="detail-featured-thumbs detail-thumbs" aria-label="Mídias do produto">${thumbs}</div>`;enhance();const stage=box.querySelector('.detail-featured-stage');let featuredIndex=0,swipeStartX=null;const activate=index=>{index=(index+galleryItems.length)%galleryItems.length;featuredIndex=index;const item=galleryItems[index];box.querySelectorAll('[data-featured-index]').forEach(button=>button.classList.toggle('active',Number(button.dataset.featuredIndex)===index));const dotHost=box.querySelector('.detail-gallery-dots');dotHost?.setAttribute('aria-label',`Mídia ${index+1} de ${galleryItems.length}`);dotHost?.querySelectorAll('span').forEach((dot,dotIndex)=>dot.classList.toggle('active',dotIndex===index));if(item.type==='video'){stage.innerHTML=`<div class="detail-featured-video"><video src="${safe(url(item))}" ${cover?`poster="${safe(url(cover,960))}"`:''} autoplay muted playsinline controls preload="auto" aria-label="${safe(item.altText||`Vídeo de ${data.name||'produto'}`)}"></video></div>`;const video=stage.querySelector('video'),play=()=>{try{video.currentTime=Math.min(start,Math.max(0,(video.duration||Infinity)-.1))}catch{}video.play().catch(()=>{})};if(video.readyState>=1)play();else video.addEventListener('loadedmetadata',play,{once:true})}else{const source={src:url(item,960),srcset:[320,640,960,1280].map(width=>url(item,width)+' '+width+'w').join(', ')};stage.innerHTML=`<button class="detail-featured-image" type="button" aria-label="Ampliar imagem" style="border:0!important;outline:0!important;box-shadow:none!important;background:transparent!important;appearance:none!important"><img src="${source.src}" srcset="${source.srcset}" sizes="(max-width:760px) calc(100vw - 44px), (max-width:1200px) 56vw, 720px" alt="${safe(item.altText||data.name||'Produto')}" decoding="async"></button>`;const imageButton=stage.querySelector('button'),imageNode=imageButton?.querySelector('img');enableImageMagnifier(imageButton,imageNode);imageButton.onclick=()=>openGallery(imageItems,imageItems.indexOf(item),data.name)}};const featuredThumbs=box.querySelector('.detail-featured-thumbs');featuredThumbs.addEventListener('click',event=>{const button=event.target.closest('[data-featured-index]');if(button)activate(Number(button.dataset.featuredIndex))});featuredThumbs.addEventListener('mouseover',event=>{if(!matchMedia('(hover:hover) and (pointer:fine)').matches)return;const button=event.target.closest('[data-featured-index]');if(!button||button.contains(event.relatedTarget))return;activate(Number(button.dataset.featuredIndex))});stage.style.touchAction='pan-y pinch-zoom';stage.addEventListener('pointerdown',event=>{swipeStartX=event.clientX},{passive:true});stage.addEventListener('pointerup',event=>{if(swipeStartX==null)return;const distance=event.clientX-swipeStartX;swipeStartX=null;if(Math.abs(distance)>=42)activate(featuredIndex+(distance<0?1:-1))},{passive:true});stage.addEventListener('pointercancel',()=>{swipeStartX=null},{passive:true});activate(0);renderProductNews(data,box);return}box.classList.toggle('has-multiple-media',items.length>1);
  ...
  const thumbList=box.querySelector('.detail-thumbs');thumbList?.addEventListener('click',event=>{const button=event.target.closest('[data-index]');if(!button)return;const index=Number(button.dataset.index);if(button.classList.contains('more-images')){openGallery(items,index,data.name);return}showMain(index)});thumbList?.addEventListener('mouseover',event=>{if(!matchMedia('(hover:hover) and (pointer:fine)').matches)return;const button=event.target.closest('[data-index]');if(!button||button.contains(event.relatedTarget))return;showMain(Number(button.dataset.index))});
  renderProductNews(data,box);
}
