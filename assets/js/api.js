...
async function request(path,{mock,method='GET',body,signal}={}){const key=method+path;if(method==='GET'&&cache.has(key))return cache.get(key);const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),C.REQUEST_TIMEOUT);signal?.addEventListener('abort',()=>ctrl.abort());try{const url=C.USE_MOCK_DATA?mock:`${C.API_BASE_URL}${path}`;if(!url)throw new Error('API não configurada');const res=await fetch(url,{method,body:body instanceof FormData?body:body?JSON.stringify(body):undefined,headers:{...userAuthorization(),...(body&&!(body instanceof FormData)?{'Content-Type':'application/json'}:{})},credentials:'include',signal:ctrl.signal});if(!res.ok)throw new Error(`Falha na requisição (${res.status})`);const json=await res.json(),data=json.data??json;if(method==='GET')cache.set(key,data);return data}finally{clearTimeout(timer)}}
