...
export const getProducts=async({store='',limit=50}={})=>withActivePromotions(await request(`/api/v1/products?${new URLSearchParams({store,limit:String(Math.min(50,Math.max(1,Number(limit)||50)))})}`,{mock:'assets/mock/products.json'}));
...
export const getHomeData=()=>C.USE_MOCK_DATA?Promise.all([getProducts({limit:50}),getTrendingProducts(16),getPromotions(),getCategories(),getFeaturedCollections(),getSiteConfig()]).then(([products,trending,campaigns,categories,collections,siteConfig])=>({products,trending,campaigns,categories,collections,siteConfig})):request('/api/v1/home').catch(()=>Promise.all([getProducts({limit:50}),getTrendingProducts(16),getPromotions(),getCategories(),getFeaturedCollections(),getSiteConfig()]).then(([products,trending,campaigns,categories,collections,siteConfig])=>({products,trending,campaigns,categories,collections,siteConfig})));
export const getRecommendations=async(slug,{standard=false}={})=>{if(C.USE_MOCK_DATA)return(await getProducts()).filter(p=>p.slug!==slug).slice(0,standard?8:4);const products=await request(`/api/v1/products/${encodeURIComponent(slug)}/related?audience=${userAuthorization().authorization?'member':'guest'}&mode=${standard?'standard':'all'}&v=10`);return withActivePromotions(products).catch(()=>products)};
