...
const cache=new Map(),inflight=new Map(),cacheTimes=new Map();
...
async function request(path,{mock,method='GET',body,signal,timeout=C.REQUEST_TIMEOUT,cacheTtl=Infinity}={}){const key=method+path;const publicRead=method==='GET'&&!C.USE_MOCK_DATA&&!userAuthorization().authorization&&/^\/api\/v1\/(?:home|products(?:\/trending)?(?:\?[^#]*)?|products\/[^/?]+(?:\?[^#]*)?|categories(?:\/weekly-highlights)?(?:\?[^#]*)?|promotions(?:\?[^#]*)?|collections(?:\/[^/?]+)?(?:\?[^#]*)?|search\?(?!.*smart=1)|site-config(?:\?[^#]*)?)$/.test(path);if(publicRead){const detail=/^\/api\/v1\/products\/[^/?]+/.test(path),search=path.startsWith('/api/v1/search?'),freshFor=0,staleFor=detail?600000:search?120000:300000;return publicJson(key,`${C.API_BASE_URL}${path}`,{freshFor,staleFor,timeout})}if(method==='GET'){if(cache.has(key)&&Date.now()-(cacheTimes.get(key)||0)<cacheTtl)return cache.get(key);if(inflight.has(key))return inflight.get(key)}const task=(async()=>{const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),timeout);signal?.addEventListener('abort',()=>ctrl.abort(),{once:true});try{const url=C.USE_MOCK_DATA?mock:`${C.API_BASE_URL}${path}`;if(!url)throw new Error('API não configurada');const res=await fetch(url,{method,body:body instanceof FormData?body:body?JSON.stringify(body):undefined,headers:{...userAuthorization(),...(body&&!(body instanceof FormData)?{'Content-Type':'application/json'}:{})},credentials:'include',signal:ctrl.signal});if(!res.ok)throw new Error(`Falha na requisição (${res.status})`);const json=await res.json(),data=json.data??json;if(method==='GET'){cache.set(key,data);cacheTimes.set(key,Date.now())}else if(/^\/api\/v1\/admin\/(?:products|categories|collections|promotions|taxonomy|catalog-settings|site-config|banners|shoplab-ads)/.test(path))invalidatePublicCache('api/v1/');return data}finally{clearTimeout(timer)}})();if(method==='GET')inflight.set(key,task);try{return await task}finally{if(method==='GET'&&inflight.get(key)===task)inflight.delete(key)}}
...
export const createProduct=async data=>{const value=await request('/api/v1/admin/products',{method:'POST',body:data});invalidatePublicCache('api/v1/');return value};
export const updateProduct=async(id,data)=>{const value=await request(`/api/v1/admin/products/${encodeURIComponent(id)}`,{method:'PUT',body:data});invalidatePublicCache('api/v1/');return value};
...
export const getDiscovery=query=>request('/api/v1/discovery?'+query,{cacheTtl:60000});
export const adminCatalogRequest=async(path,{method='GET',body}={})=>{
  const form=body instanceof FormData;
  const response=await fetch(C.API_BASE_URL+path,{method,credentials:'include',headers:body&&!form?{'Content-Type':'application/json'}:{},body:body?(form?body:JSON.stringify(body)):undefined});
  const json=await response.json();
  if(!response.ok||!json.success)throw new Error(json.error?.message||`Erro ${response.status}`);
  return json.data;
};
