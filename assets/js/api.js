...
async function request(path,{mock,method='GET',body,signal,timeout=C.REQUEST_TIMEOUT,cacheTtl=Infinity}={}){const key=method+path;if(method==='GET'){if(cache.has(key)&&Date.now()-(cacheTimes.get(key)||0)<cacheTtl)return cache.get(key);if(inflight.has(key))return inflight.get(key)}const task=(async()=>{const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),timeout);signal?.addEventListener('abort',()=>ctrl.abort(),{once:true});try{const url=C.USE_MOCK_DATA?mock:`${C.API_BASE_URL}${path}`;if(!url)throw new Error('API não configurada');const res=await fetch(url,{method,body:body instanceof FormData?body:body?JSON.stringify(body):undefined,headers:{...userAuthorization(),...(body&&!(body instanceof FormData)?{'Content-Type':'application/json'}:{})},credentials:'include',signal:ctrl.signal});if(!res.ok)throw new Error(`Falha na requisição (${res.status})`);const json=await res.json(),data=json.data??json;if(method==='GET'){cache.set(key,data);cacheTimes.set(key,Date.now())}return data}finally{clearTimeout(timer)}})();if(method==='GET')inflight.set(key,task);try{return await task}finally{if(method==='GET'&&inflight.get(key)===task)inflight.delete(key)}}
