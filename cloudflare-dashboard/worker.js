...
async function saveShoplabAdAssignments(req,env,id){
  ...
  await ensureShoplabAdsSchema(env);
  const url=new URL(req.url),device=url.searchParams.get('device')==='mobile'?'mobile':'desktop',pageKind=['home','products','category','product'].includes(url.searchParams.get('page'))?url.searchParams.get('page'):'home',category=String(url.searchParams.get('category')||'').slice(0,100),searchQuery=normalizeSearch(String(url.searchParams.get('q')||'').slice(0,160)),origin=url.origin,user=req.headers.has('authorization')?await activeUser(req,env):null;
  const adFields=`a.id,a.name,a.description,a.product_slug productSlug,a.target_keywords targetKeywords,a.public_title publicTitle,a.ad_label adLabel,a.show_header showHeader,a.dismissible,a.dismiss_minutes dismissMinutes,a.cta_text ctaText,a.cta_color ctaColor,a.old_price_text oldPriceText,a.current_price_text currentPriceText,a.old_price_color oldPriceColor,a.current_price_color currentPriceColor,a.media_type mediaType,a.storage_key storageKey,a.link_url linkUrl,a.priority,a.distribution_mode distributionMode,a.distribution_weight distributionWeight,a.featured,a.search_boost searchBoost,a.max_per_page maxPerPage,a.target_pages targetPages,p.slug linkedProductSlug,p.name linkedProductName,pm.storage_key linkedStorageKey,pm.external_url linkedExternalUrl,COALESCE(o.current_price_cents,p.base_price_cents) linkedCurrentPrice,COALESCE(o.previous_price_cents,p.compare_at_price_cents) linkedOldPrice`;
  const manualResult=await env.DB.prepare(`SELECT ${adFields},x.position_key positionKey FROM shoplab_ad_placement_members x JOIN shoplab_ads a ON a.id=x.ad_id LEFT JOIN products p ON p.slug=a.product_slug LEFT JOIN offers o ON o.product_id=p.id AND o.is_primary=1 LEFT JOIN product_media pm ON pm.id=(SELECT id FROM product_media WHERE product_id=p.id AND type='image' ORDER BY is_primary DESC,sort_order,created_at LIMIT 1) WHERE x.device=? AND x.page_kind=? AND (x.category_slug='' OR x.category_slug=?) AND a.status='active' AND a.distribution_mode='manual' AND (a.starts_at IS NULL OR datetime(a.starts_at)<=CURRENT_TIMESTAMP) AND (a.ends_at IS NULL OR datetime(a.ends_at)>=CURRENT_TIMESTAMP)`).bind(device,pageKind,category).all();
  const automaticResult=await env.DB.prepare(`SELECT ${adFields} FROM shoplab_ads a LEFT JOIN products p ON p.slug=a.product_slug LEFT JOIN offers o ON o.product_id=p.id AND o.is_primary=1 LEFT JOIN product_media pm ON pm.id=(SELECT id FROM product_media WHERE product_id=p.id AND type='image' ORDER BY is_primary DESC,sort_order,created_at LIMIT 1) WHERE a.status='active' AND a.distribution_mode='weighted' AND (a.starts_at IS NULL OR datetime(a.starts_at)<=CURRENT_TIMESTAMP) AND (a.ends_at IS NULL OR datetime(a.ends_at)>=CURRENT_TIMESTAMP)`).all();
  let searches=[];if(user){const history=await env.DB.prepare(`SELECT query_text FROM events WHERE user_id=? AND event_type IN ('search','search_no_results') AND query_text IS NOT NULL AND created_at>=datetime('now','-60 days') ORDER BY created_at DESC LIMIT 30`).bind(user.id).all();searches=(history.results||[]).map(row=>normalizeSearch(row.query_text)).filter(Boolean)}
  const matchesFor=row=>{const terms=[normalizeSearch(row.publicTitle||row.name),...String(row.targetKeywords||'').split(',').map(normalizeSearch)].filter(Boolean),current=searchQuery?terms.reduce((score,term)=>score+(term.includes(searchQuery)||searchQuery.includes(term)?8:searchQuery.split(' ').filter(word=>word.length>2&&term.includes(word)).length*2),0):0,history=terms.reduce((total,term)=>total+searches.filter(query=>query.includes(term)||term.includes(query)).length,0);return{current,history,total:current+history}};
  const manualRanked=(manualResult.results||[]).map(row=>({...row,_match:matchesFor(row),_random:Math.random()})).sort((a,b)=>b._match.total-a._match.total||a._random-b._random);
  ...
  const automatic=(automaticResult.results||[]).filter(row=>String(row.targetPages||'').split(',').includes(pageKind)).map(row=>({...row,_match:matchesFor(row),_remaining:Math.min(Number(row.maxPerPage)||1,Math.ceil(Number(row.distributionWeight||25)/25))}));
  for(const positionKey of slots){const eligible=automatic.filter(row=>row._remaining>0);if(!eligible.length)break;const matched=searchQuery?eligible.filter(row=>row._match.current>0):[],pool=matched.length?matched:eligible;const winner=pool.map(row=>({row,score:-Math.log(Math.max(Number.EPSILON,Math.random()))/(Math.max(1,Number(row.distributionWeight))*(Number(row.featured)?1.6:1)*(1+row._match.total*Math.max(1,Number(row.searchBoost)||3)))})).sort((a,b)=>a.score-b.score)[0].row;winner._remaining--;winners.push({...winner,positionKey})}
  const adPrice=value=>value==null?'':'R$ '+(Number(value)/100).toFixed(2).replace('.',',');
  return ok(req,env,winners.map(({targetKeywords,priority,distributionMode,_match,_random,_remaining,...row})=>({...row,searchMatch:Boolean(_match?.current),linkUrl:row.linkedProductSlug?'produto.html?slug='+encodeURIComponent(row.linkedProductSlug):row.linkUrl,oldPriceText:row.linkedProductSlug&&row.linkedOldPrice?adPrice(row.linkedOldPrice):row.oldPriceText,currentPriceText:row.linkedProductSlug&&row.linkedCurrentPrice?adPrice(row.linkedCurrentPrice):row.currentPriceText,personalized:Boolean(_match?.total),mediaUrl:row.storageKey?`${origin}/media/${encodeURIComponent(row.storageKey)}`:row.linkedStorageKey?`${origin}/media/${encodeURIComponent(row.linkedStorageKey)}`:row.linkedExternalUrl||null,storageKey:undefined,linkedStorageKey:undefined,linkedExternalUrl:undefined})),id)
