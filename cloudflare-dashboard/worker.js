...
async function legacyPublicShoplabAds(req,env,id){
  await ensureShoplabAdsSchema(env);
  const url=new URL(req.url),device=url.searchParams.get('device')==='mobile'?'mobile':'desktop',pageKind=['home','products','category','product','news'].includes(url.searchParams.get('page'))?url.searchParams.get('page'):'home',category=String(url.searchParams.get('category')||'').slice(0,100),newsPosition=pageKind==='news'&&url.searchParams.get('surface')==='article'?'news_article':pageKind==='news'?'news_feed':'',origin=url.origin,user=req.headers.has('authorization')?await activeUser(req,env):null;
...
async function saveShoplabAdAssignments(req,env,id){
  ...
  const automaticResult=await env.DB.prepare(`SELECT ${adFields} FROM shoplab_ads a LEFT JOIN products p ON p.slug=a.product_slug LEFT JOIN offers o ON o.product_id=p.id AND o.is_primary=1 WHERE a.status='active' AND a.distribution_mode='weighted' AND a.distribution_weight>=? AND instr(','||replace(a.target_pages,' ','')||',',','||?||',')>0 AND (a.starts_at IS NULL OR datetime(a.starts_at)<=CURRENT_TIMESTAMP) AND (a.ends_at IS NULL OR datetime(a.ends_at)>=CURRENT_TIMESTAMP)`).bind(({home:1,products:26,category:51,product:76,news:1})[pageKind],pageKind).all();
  ...
  const manualRanked=(manualResult.results||[]).filter(row=>!newsPosition||row.positionKey===newsPosition).map(row=>({...row,_matches:matchesFor(row),_random:Math.random()})).sort((a,b)=>b._matches-a._matches||a._random-b._random);
