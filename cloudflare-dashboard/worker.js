...
async function route(request, env, ctx, requestId) {
  ...
  if (['GET','PUT'].includes(request.method) && path === '/api/v1/admin/shoplab-ads/settings') return adminShoplabAdsSettings(request, env, requestId);
...
async function ensureShoplabAdsSchema(env){
  await env.DB.batch([
    ...
    env.DB.prepare(`INSERT OR IGNORE INTO shoplab_ads_settings(key,value) VALUES('global_limits','{"home":2,"search":1,"category":1,"product":1,"products":2,"minimumOrganicDesktop":6,"minimumOrganicMobile":8}')`),
    env.DB.prepare(`UPDATE shoplab_ads_settings SET value='{"home":2,"search":1,"category":1,"product":1,"products":2,"minimumOrganicDesktop":6,"minimumOrganicMobile":8}' WHERE key='global_limits' AND value='{"home":2,"search":1,"category":2,"product":1,"products":2}'`)
  ]);
  ...
  for(const [column,definition] of [['position_index','INTEGER'],['page_url','TEXT']]){try{await env.DB.prepare(`ALTER TABLE shoplab_ad_events ADD COLUMN ${column} ${definition}`).run()}catch(error){if(!new RegExp(`duplicate column name:.*${column}`,'i').test(String(error?.message||error)))throw error}}
...
async function adminShoplabAds(req,env,id){
  ...
  const {results}=await env.DB.prepare(`SELECT id,name,description,product_slug productSlug,target_keywords targetKeywords,public_title publicTitle,ad_label adLabel,show_header showHeader,dismissible,dismiss_minutes dismissMinutes,cta_text ctaText,cta_color ctaColor,old_price_text oldPriceText,current_price_text currentPriceText,old_price_color oldPriceColor,current_price_color currentPriceColor,media_type mediaType,storage_key storageKey,link_url linkUrl,status,priority,distribution_mode distributionMode,distribution_weight distributionWeight,featured,search_boost searchBoost,max_per_page maxPerPage,target_pages targetPages,starts_at startsAt,ends_at endsAt,no_end_date noEndDate,category_slugs categorySlugs,related_product_slugs relatedProductSlugs,CASE WHEN status='draft' THEN 'draft' WHEN status='paused' THEN 'paused' WHEN starts_at IS NOT NULL AND datetime(starts_at)>CURRENT_TIMESTAMP THEN 'scheduled' WHEN no_end_date=0 AND ends_at IS NOT NULL AND datetime(ends_at)<CURRENT_TIMESTAMP THEN 'ended' ELSE 'active' END derivedStatus FROM shoplab_ads ORDER BY priority DESC,created_at DESC`).all();
  return ok(req,env,results||[],id)
}async function recordShoplabAdEvent(req,env,id){
  await ensureShoplabAdsSchema(env);const body=await readJson(req,6000),eventId=String(body.eventId||'').slice(0,100),adId=String(body.adId||'').slice(0,100),eventType=String(body.eventType||''),device=['mobile','tablet'].includes(body.device)?body.device:'desktop',pageKind=['home','search','products','category','product'].includes(body.pageKind)?body.pageKind:'home',positionKey=String(body.placementId||body.positionKey||'unknown').slice(0,80),sessionId=String(body.sessionId||'').slice(0,100),positionIndex=clamp(body.positionIndex,0,1000,0),pageUrl=String(body.pageUrl||'').slice(0,500);
  ...
  await env.DB.prepare(`INSERT OR IGNORE INTO shoplab_ad_events(id,ad_id,event_type,device,page_kind,position_key,session_id,position_index,page_url) VALUES(?,?,?,?,?,?,?,?,?)`).bind(eventId,adId,eventType,device,pageKind,positionKey,sessionId||null,positionIndex,pageUrl||null).run();return ok(req,env,{recorded:true},id)
}async function adminShoplabAdsAnalytics(req,env,id){
  if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);const url=new URL(req.url),days=clamp(url.searchParams.get('days'),7,90,30),since=`-${days-1} days`,uniqueExpr=`CASE WHEN session_id IS NOT NULL AND session_id<>'' THEN session_id||':'||ad_id||':'||position_key END`;
  const [totalsResult,dailyResult,adsResult,devicesResult,pagesResult,placementsResult]=await Promise.all([
    env.DB.prepare(`SELECT SUM(event_type='impression') impressions,SUM(event_type='click') clicks,COUNT(DISTINCT CASE WHEN event_type='impression' THEN ${uniqueExpr} END) uniqueImpressions,COUNT(DISTINCT CASE WHEN event_type='click' THEN ${uniqueExpr} END) uniqueClicks FROM shoplab_ad_events WHERE created_at>=datetime('now',?)`).bind(since).first(),
    env.DB.prepare(`SELECT date(created_at) day,SUM(event_type='impression') impressions,SUM(event_type='click') clicks FROM shoplab_ad_events WHERE created_at>=datetime('now',?) GROUP BY date(created_at) ORDER BY day`).bind(since).all(),
    env.DB.prepare(`SELECT a.id,a.name,a.status,SUM(e.event_type='impression') impressions,SUM(e.event_type='click') clicks,COUNT(DISTINCT CASE WHEN e.event_type='impression' THEN e.session_id||':'||e.ad_id||':'||e.position_key END) uniqueImpressions,COUNT(DISTINCT CASE WHEN e.event_type='click' THEN e.session_id||':'||e.ad_id||':'||e.position_key END) uniqueClicks FROM shoplab_ads a LEFT JOIN shoplab_ad_events e ON e.ad_id=a.id AND e.created_at>=datetime('now',?) GROUP BY a.id ORDER BY impressions DESC`).bind(since).all(),
    env.DB.prepare(`SELECT device,SUM(event_type='impression') impressions,SUM(event_type='click') clicks,COUNT(DISTINCT CASE WHEN event_type='impression' THEN ${uniqueExpr} END) uniqueImpressions,COUNT(DISTINCT CASE WHEN event_type='click' THEN ${uniqueExpr} END) uniqueClicks FROM shoplab_ad_events WHERE created_at>=datetime('now',?) GROUP BY device`).bind(since).all(),
    env.DB.prepare(`SELECT page_kind pageKind,SUM(event_type='impression') impressions,SUM(event_type='click') clicks FROM shoplab_ad_events WHERE created_at>=datetime('now',?) GROUP BY page_kind`).bind(since).all(),
    env.DB.prepare(`SELECT position_key placementId,SUM(event_type='impression') impressions,SUM(event_type='click') clicks,COUNT(DISTINCT CASE WHEN event_type='impression' THEN ${uniqueExpr} END) uniqueImpressions,COUNT(DISTINCT CASE WHEN event_type='click' THEN ${uniqueExpr} END) uniqueClicks FROM shoplab_ad_events WHERE created_at>=datetime('now',?) GROUP BY position_key ORDER BY impressions DESC`).bind(since).all()
  ]);
  ...
  const totals=decorate(totalsResult||{});return ok(req,env,{days,totals,daily:(dailyResult.results||[]).map(decorate),ads:(adsResult.results||[]).map(decorate),devices:(devicesResult.results||[]).map(decorate),pages:(pagesResult.results||[]).map(decorate),placements:(placementsResult.results||[]).map(decorate)},id)
}
async function adminShoplabAdsSettings(req,env,id){
  if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);
  if(req.method==='PUT'){const body=await readJson(req,4000),limits={home:clamp(body.home,0,2,2),search:clamp(body.search,0,1,1),category:clamp(body.category,0,1,1),product:clamp(body.product,0,1,1),products:clamp(body.products,0,2,2),minimumOrganicDesktop:clamp(body.minimumOrganicDesktop,2,20,6),minimumOrganicMobile:clamp(body.minimumOrganicMobile,2,24,8)};await env.DB.prepare(`INSERT INTO shoplab_ads_settings(key,value,updated_at) VALUES('global_limits',?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP`).bind(JSON.stringify(limits)).run();return ok(req,env,limits,id)}
  const row=await env.DB.prepare(`SELECT value FROM shoplab_ads_settings WHERE key='global_limits'`).first();let limits={home:2,search:1,category:1,product:1,products:2,minimumOrganicDesktop:6,minimumOrganicMobile:8};try{limits={...limits,...JSON.parse(row?.value||'{}')}}catch{}return ok(req,env,limits,id)
}
function shoplabCsv(form,key,limit=50){return String(form.get(key)||'').split(/[;,\n]/).map(value=>normalizeSearch(value).replace(/\s+/g,'-')).filter(Boolean).filter((value,index,items)=>items.indexOf(value)===index).slice(0,limit).join(',')}async function createShoplabAd(req,env,id){
  if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);if(!env.MEDIA)return fail(req,env,'R2_NOT_CONFIGURED','Mídia não configurada',503,id);const form=await req.formData(),name=String(form.get('name')||'').trim().slice(0,140),link=validDestination(form.get('linkUrl')),status=['draft','active','paused'].includes(form.get('status'))?form.get('status'):'draft',file=form.get('file'),presentation=shoplabAdPresentation(form,name),targetKeywords=shoplabAdKeywords(form),startsAt=String(form.get('startsAt')||'').trim()||null,noEndDate=form.get('noEndDate')==='0'?0:1,endsAt=noEndDate?null:String(form.get('endsAt')||'').trim()||null,categorySlugs=shoplabCsv(form,'categorySlugs'),relatedProductSlugs=shoplabCsv(form,'relatedProductSlugs');if(!name||!link)return fail(req,env,'VALIDATION_ERROR','Informe nome e destino válidos',422,id);const adId=crypto.randomUUID();let storageKey=null;try{if(file instanceof File&&file.size)storageKey=await storeHeaderAdMedia(env,file,adId)}catch(error){return fail(req,env,'INVALID_FILE',error.message,422,id)}const mediaType=file instanceof File&&file.type.startsWith('video/')?'video':'image';await env.DB.prepare(`INSERT INTO shoplab_ads(id,name,description,product_slug,target_keywords,public_title,ad_label,show_header,dismissible,dismiss_minutes,cta_text,cta_color,old_price_text,current_price_text,old_price_color,current_price_color,media_type,storage_key,link_url,status,priority,distribution_mode,distribution_weight,featured,search_boost,max_per_page,target_pages,starts_at,ends_at,no_end_date,category_slugs,related_product_slugs) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(adId,name,String(form.get('description')||'').slice(0,240),String(form.get('productSlug')||'').trim().slice(0,140)||null,targetKeywords,presentation.publicTitle,presentation.adLabel,presentation.showHeader,presentation.dismissible,presentation.dismissMinutes,presentation.ctaText,presentation.ctaColor,presentation.oldPriceText,presentation.currentPriceText,presentation.oldPriceColor,presentation.currentPriceColor,mediaType,storageKey,link,status,clamp(form.get('priority'),1,1000,1),'weighted',clamp(form.get('distributionWeight'),1,100,25),form.get('featured')==='1'?1:0,clamp(form.get('searchBoost'),1,10,3),clamp(form.get('maxPerPage'),1,4,1),String(form.get('targetPages')||'home,products,category,product').split(',').filter(value=>['home','products','category','product'].includes(value)).join(',')||'home,products,category,product',startsAt,endsAt,noEndDate,categorySlugs,relatedProductSlugs).run();return ok(req,env,{id:adId,storageKey},id)
}
async function updateShoplabAd(req,env,adId,id){
  if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);const current=await env.DB.prepare(`SELECT storage_key storageKey,media_type mediaType FROM shoplab_ads WHERE id=?`).bind(adId).first();if(!current)return fail(req,env,'AD_NOT_FOUND','Anúncio não encontrado',404,id);const form=await req.formData(),name=String(form.get('name')||'').trim().slice(0,140),link=validDestination(form.get('linkUrl')),status=['draft','active','paused'].includes(form.get('status'))?form.get('status'):'draft',file=form.get('file'),presentation=shoplabAdPresentation(form,name),targetKeywords=shoplabAdKeywords(form),startsAt=String(form.get('startsAt')||'').trim()||null,noEndDate=form.get('noEndDate')==='0'?0:1,endsAt=noEndDate?null:String(form.get('endsAt')||'').trim()||null,categorySlugs=shoplabCsv(form,'categorySlugs'),relatedProductSlugs=shoplabCsv(form,'relatedProductSlugs');if(!name||!link)return fail(req,env,'VALIDATION_ERROR','Informe nome e destino válidos',422,id);let storageKey=current.storageKey,mediaType=current.mediaType;try{if(file instanceof File&&file.size){if(!env.MEDIA)return fail(req,env,'R2_NOT_CONFIGURED','Mídia não configurada',503,id);storageKey=await storeHeaderAdMedia(env,file,adId);mediaType=file.type.startsWith('video/')?'video':'image'}}catch(error){return fail(req,env,'INVALID_FILE',error.message,422,id)}await env.DB.prepare(`UPDATE shoplab_ads SET name=?,description=?,product_slug=?,target_keywords=?,public_title=?,ad_label=?,show_header=?,dismissible=?,dismiss_minutes=?,cta_text=?,cta_color=?,old_price_text=?,current_price_text=?,old_price_color=?,current_price_color=?,media_type=?,storage_key=?,link_url=?,status=?,priority=?,distribution_mode='weighted',distribution_weight=?,featured=?,search_boost=?,max_per_page=?,target_pages=?,starts_at=?,ends_at=?,no_end_date=?,category_slugs=?,related_product_slugs=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(name,String(form.get('description')||'').slice(0,240),String(form.get('productSlug')||'').trim().slice(0,140)||null,targetKeywords,presentation.publicTitle,presentation.adLabel,presentation.showHeader,presentation.dismissible,presentation.dismissMinutes,presentation.ctaText,presentation.ctaColor,presentation.oldPriceText,presentation.currentPriceText,presentation.oldPriceColor,presentation.currentPriceColor,mediaType,storageKey,link,status,clamp(form.get('priority'),1,1000,1),clamp(form.get('distributionWeight'),1,100,25),form.get('featured')==='1'?1:0,clamp(form.get('searchBoost'),1,10,3),clamp(form.get('maxPerPage'),1,4,1),String(form.get('targetPages')||'home,products,category,product').split(',').filter(value=>['home','products','category','product'].includes(value)).join(',')||'home,products,category,product',startsAt,endsAt,noEndDate,categorySlugs,relatedProductSlugs,adId).run();return ok(req,env,{id:adId,storageKey,status},id)
...
async function saveShoplabAdAssignments(req,env,id){
  if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);
  const body=await readJson(req,120000),items=Array.isArray(body.items)?body.items.slice(0,200):[],devices=new Set(['desktop','mobile']),pages=new Set(['home','products','category','product']),valid=items.filter(item=>devices.has(item.device)&&pages.has(item.pageKind)&&item.adId&&item.positionKey);
  ...
  try{
    await ensureShoplabAdsSchema(env);const url=new URL(req.url),requestedPage=String(url.searchParams.get('page')||'home'),pageKind=['home','search','products','category','product'].includes(requestedPage)?requestedPage:'home',targetPage=pageKind==='search'?'products':pageKind,device=['mobile','tablet'].includes(url.searchParams.get('device'))?url.searchParams.get('device'):'desktop',category=normalizeSearch(String(url.searchParams.get('category')||'').slice(0,100)).replace(/\s+/g,'-'),product=normalizeSearch(String(url.searchParams.get('product')||'').slice(0,140)).replace(/\s+/g,'-'),searchQuery=normalizeSearch(String(url.searchParams.get('q')||'').slice(0,160)),candidateMode=url.searchParams.get('candidates')==='1',seenIds=new Set(String(url.searchParams.get('seen')||'').split(',').map(value=>value.slice(0,100)).filter(Boolean).slice(0,20)),origin=url.origin;
    const setting=await env.DB.prepare(`SELECT value FROM shoplab_ads_settings WHERE key='global_limits'`).first();let limits={home:2,search:1,category:1,product:1,products:2,minimumOrganicDesktop:6,minimumOrganicMobile:8};try{limits={...limits,...JSON.parse(setting?.value||'{}')}}catch{}const hardLimits={home:2,search:1,category:1,product:1,products:2},globalLimit=clamp(limits[pageKind],0,hardLimits[pageKind]||1,limits[pageKind]??1);if(!globalLimit)return ok(req,env,[],id);const currentProduct=product?await env.DB.prepare(`SELECT c.slug categorySlug FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.slug=?`).bind(product).first():null,effectiveCategory=category||currentProduct?.categorySlug||'';
    const adFields=`a.id,a.name,a.description,a.product_slug productSlug,a.target_keywords targetKeywords,a.public_title publicTitle,a.ad_label adLabel,a.show_header showHeader,a.dismissible,a.dismiss_minutes dismissMinutes,a.cta_text ctaText,a.cta_color ctaColor,a.old_price_text oldPriceText,a.current_price_text currentPriceText,a.old_price_color oldPriceColor,a.current_price_color currentPriceColor,a.media_type mediaType,a.storage_key storageKey,a.link_url linkUrl,a.priority,a.distribution_weight distributionWeight,a.featured,a.search_boost searchBoost,a.max_per_page maxPerPage,a.target_pages targetPages,a.category_slugs categorySlugs,a.related_product_slugs relatedProductSlugs,p.slug linkedProductSlug,p.name linkedProductName,c.slug linkedCategorySlug,c.name linkedCategoryName,pm.storage_key linkedStorageKey,pm.external_url linkedExternalUrl,COALESCE(o.current_price_cents,p.base_price_cents) linkedCurrentPrice,COALESCE(o.previous_price_cents,p.compare_at_price_cents) linkedOldPrice`;
    const result=await env.DB.prepare(`SELECT ${adFields} FROM shoplab_ads a LEFT JOIN products p ON p.slug=a.product_slug LEFT JOIN categories c ON c.id=p.category_id LEFT JOIN offers o ON o.product_id=p.id AND o.is_primary=1 LEFT JOIN product_media pm ON pm.id=(SELECT id FROM product_media WHERE product_id=p.id AND type='image' ORDER BY is_primary DESC,sort_order,created_at LIMIT 1) WHERE a.status='active' AND (a.starts_at IS NULL OR datetime(a.starts_at)<=CURRENT_TIMESTAMP) AND (a.no_end_date=1 OR a.ends_at IS NULL OR datetime(a.ends_at)>=CURRENT_TIMESTAMP)`).all();
    const performance=await env.DB.prepare(`SELECT ad_id adId,SUM(event_type='impression') impressions,SUM(event_type='click') clicks FROM shoplab_ad_events WHERE created_at>=datetime('now','-30 days') AND page_kind=? GROUP BY ad_id`).bind(pageKind).all(),performanceRows=performance.results||[],performanceMap=new Map(performanceRows.map(row=>[row.adId,row])),performanceImpressions=performanceRows.reduce((sum,row)=>sum+Number(row.impressions||0),0),placementCtr=performanceImpressions?performanceRows.reduce((sum,row)=>sum+Number(row.clicks||0),0)/performanceImpressions:0;
    const tokens=value=>normalizeSearch(value||'').split(/\s+/).filter(word=>word.length>1),queryTokens=tokens(searchQuery);
    const relevance=row=>{const title=normalizeSearch(row.publicTitle||''),keywords=String(row.targetKeywords||'').split(',').map(normalizeSearch),productName=normalizeSearch(row.linkedProductName||''),categoryName=normalizeSearch(row.linkedCategoryName||''),internal=normalizeSearch(row.name||''),fields=[[title,12],[productName,11],[categoryName,8],...keywords.map(value=>[value,10]),...String(row.categorySlugs||'').split(',').map(value=>[normalizeSearch(value),8]),...String(row.relatedProductSlugs||'').split(',').map(value=>[normalizeSearch(value),9]),[internal,2]];let search=0;if(searchQuery)for(const [value,weight] of fields){if(!value)continue;if(value===searchQuery)search+=weight*4;else if(value.startsWith(searchQuery)||searchQuery.startsWith(value))search+=weight*2;else search+=queryTokens.reduce((score,term)=>score+(value.split(/\s+/).some(word=>word===term)?weight:wordStarts(value,term)?weight*.65:value.includes(term)?weight*.35:0),0)}const categories=String(row.categorySlugs||'').split(',').filter(Boolean),products=String(row.relatedProductSlugs||'').split(',').filter(Boolean);let context=search;if(effectiveCategory&&(categories.includes(effectiveCategory)||row.linkedCategorySlug===effectiveCategory))context+=35;if(product&&(products.includes(product)||row.linkedProductSlug===product))context+=55;return{search,context}};
    function wordStarts(value,term){return value.split(/\s+/).some(word=>word.startsWith(term)||term.startsWith(word))}
    let eligible=(result.results||[]).filter(row=>String(row.targetPages||'').split(',').includes(targetPage)).map(row=>{const context=relevance(row),stats=performanceMap.get(row.id),impressions=Number(stats?.impressions)||0,ctr=impressions?Number(stats?.clicks||0)/impressions:0,performanceMultiplier=impressions<200?1:Math.max(.9,Math.min(1.15,1+(ctr-placementCtr)*2.5));return{...row,_context:context,_remaining:Math.min(Number(row.maxPerPage)||1,Math.ceil(Number(row.distributionWeight||25)/25)),_performance:performanceMultiplier,_seen:seenIds.has(row.id)}});
    if(searchQuery)eligible=eligible.filter(row=>row._context.search>=8);if(pageKind==='category')eligible=eligible.filter(row=>row._context.context>=20);if(pageKind==='product')eligible=eligible.filter(row=>row._context.context>=15);if(!eligible.length)return ok(req,env,[],id);eligible.sort((a,b)=>b._context.context-a._context.context);const bestContext=eligible[0]._context.context;if(searchQuery)eligible=eligible.filter(row=>row._context.context>=Math.max(8,bestContext*.55));
    const placements={home:['home_trending','home_offers'],search:['search_results'],products:['products_grid','products_more'],category:['category_grid'],product:['product_middle']}[pageKind],winners=[],selectionLimit=candidateMode?Math.min(8,eligible.length):Math.min(globalLimit,placements.length);
    for(let index=0;index<selectionLimit;index++){const pool=eligible.filter(row=>row._remaining>0);if(!pool.length)break;const explore=Math.random()<.15,weighted=pool.map(row=>{const contextMultiplier=searchQuery?1+Math.min(4,(row._context.search*Math.max(1,Number(row.searchBoost)||3)/3)/12):1+Math.min(2,row._context.context/35),performanceMultiplier=explore?1:row._performance,weight=Math.max(1,Number(row.distributionWeight)||25)*(Number(row.featured)?1.6:1)*contextMultiplier*performanceMultiplier*(row._seen?.35:1);return{row,key:-Math.log(Math.max(Number.EPSILON,Math.random()))/weight}}).sort((a,b)=>a.key-b.key);const winner=weighted[0].row;winner._remaining--;winners.push({...winner,positionKey:placements[index%placements.length],placementId:placements[index%placements.length],positionIndex:index})}
    const adPrice=value=>value==null?'':'R$ '+(Number(value)/100).toFixed(2).replace('.',',');return ok(req,env,winners.map(({targetKeywords,priority,_context,_remaining,_performance,_seen,...row})=>({...row,searchMatch:Boolean(searchQuery),minimumOrganicItems:device==='mobile'?limits.minimumOrganicMobile:limits.minimumOrganicDesktop,linkUrl:row.linkedProductSlug?'produto.html?slug='+encodeURIComponent(row.linkedProductSlug):row.linkUrl,oldPriceText:row.linkedProductSlug&&row.linkedOldPrice?adPrice(row.linkedOldPrice):row.oldPriceText,currentPriceText:row.linkedProductSlug&&row.linkedCurrentPrice?adPrice(row.linkedCurrentPrice):row.currentPriceText,mediaUrl:row.storageKey?`${origin}/media/${encodeURIComponent(row.storageKey)}`:row.linkedStorageKey?`${origin}/media/${encodeURIComponent(row.linkedStorageKey)}`:row.linkedExternalUrl||null,storageKey:undefined,linkedStorageKey:undefined,linkedExternalUrl:undefined})),id)
  }catch(error){return ok(req,env,[],id)}
