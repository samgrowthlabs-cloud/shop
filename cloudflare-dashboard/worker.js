...
function trustedDeviceCookie(response,token,maxAge){response.headers.append('set-cookie',`shoplab_trusted_device=${token||''}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${maxAge}`);return response}
...
async function validHybridGrant(req,env,ip){const settings=await env.DB.prepare(`SELECT access_mode accessMode FROM admin_security_settings WHERE id='global'`).first();if(settings?.accessMode!=='hybrid')return false;const token=cookie(req,'shoplab_trusted_device'),session=cookie(req,'shoplab_session');if(!token||!session)return false;const row=await env.DB.prepare(`SELECT d.id deviceId,d.actor_id actorId,d.last_ip lastIp FROM admin_trusted_devices d JOIN admin_sessions s ON s.token_hash=? LEFT JOIN admin_collaborators c ON d.actor_type='collaborator' AND c.id=d.actor_id WHERE d.token_hash=? AND d.revoked_at IS NULL AND datetime(d.expires_at)>CURRENT_TIMESTAMP AND datetime(s.expires_at)>CURRENT_TIMESTAMP AND ((d.actor_type='owner' AND d.actor_id='owner' AND s.collaborator_id IS NULL) OR (d.actor_type='collaborator' AND s.collaborator_id=d.actor_id AND c.is_active=1)) LIMIT 1`).bind(await sha256(session),await sha256(token)).first();if(!row)return false;if(ip&&row.lastIp!==ip)await deviceHistory(env,row.deviceId,row.actorId,ip,'NEW_NETWORK_DETECTED','observed',{previousIp:row.lastIp||null});await env.DB.prepare(`UPDATE admin_trusted_devices SET last_used_at=CURRENT_TIMESTAMP,last_ip=? WHERE id=?`).bind(ip||row.lastIp,row.deviceId).run();return true}async function verifyActorReauth(env,actor,password){if(actor.role==='owner')return safeEqual(String(password||''),String(env.ADMIN_PASSWORD||''));const row=await env.DB.prepare(`SELECT password_salt salt,password_hash hash FROM admin_collaborators WHERE id=? AND is_active=1`).bind(actor.id).first();return Boolean(row)&&verifyAdminPassword(String(password||''),row.salt,row.hash)}
async function adminAccessStatus(req,env,id){const ip=trustedClientIp(req),device=await trustedDeviceForRequest(req,env).catch(()=>null),actor=await adminActor(req,env).catch(()=>null),settings=await env.DB.prepare(`SELECT access_mode accessMode FROM admin_security_settings WHERE id='global'`).first(),entries=ip?await activeAdminIpEntries(env).catch(()=>[]):[],networkAuthorized=Boolean(ip&&(ipMatchesAny(ip,recoveryNetworks(env))||entries.some(entry=>ipMatchesAny(ip,[entry.network])))),deviceMatchesActor=Boolean(device&&actor&&device.actor_id===actor.id&&device.actor_type===(actor.role==='owner'?'owner':'collaborator'));if(deviceMatchesActor&&ip&&device.last_ip!==ip){await deviceHistory(env,device.id,actor.id,ip,'NEW_NETWORK_DETECTED','observed',{previousIp:device.last_ip||null});await env.DB.prepare(`UPDATE admin_trusted_devices SET last_used_at=CURRENT_TIMESTAMP,last_ip=? WHERE id=?`).bind(ip,device.id).run()}return ok(req,env,{accessMode:settings?.accessMode||'strict',currentIp:ip,networkAuthorized,device:device?{id:device.id,name:device.name,lastIp:ip||device.last_ip,expiresAt:device.expires_at}:null,authenticated:Boolean(actor),deviceMatchesActor,actor:actor?{id:actor.id,name:actor.name,role:actor.role}:null},id)}async function grantNewNetworkAccess(req,env,id){const body=await readJson(req,8000),actor=await adminActor(req,env),ip=trustedClientIp(req);if(!actor)return fail(req,env,'UNAUTHORIZED','Valid administrative login required',401,id);const settings=await env.DB.prepare(`SELECT access_mode accessMode,allow_collaborator_permanent_network allowPermanent FROM admin_security_settings WHERE id='global'`).first();if(settings?.accessMode!=='hybrid')return fail(req,env,'STRICT_ACCESS_MODE','Trusted-device recovery is disabled in strict mode',403,id);const device=await trustedDeviceForRequest(req,env);if(!device||device.actor_id!==actor.id||device.actor_type!==(actor.role==='owner'?'owner':'collaborator'))return fail(req,env,'DEVICE_VALIDATION_FAILED','Trusted device does not belong to this user',403,id);if(!(await verifyActorReauth(env,actor,body.password)))return fail(req,env,'REAUTHENTICATION_REQUIRED','Confirm your current password',401,id);if(body.permanent){if(actor.role!=='owner'&&!Number(settings.allowPermanent))return fail(req,env,'OWNER_APPROVAL_REQUIRED','Only the owner may permanently authorize a network',403,id);await env.DB.prepare(`INSERT INTO admin_ip_allowlist(id,network,name,description,access_type,collaborator_id,created_by) VALUES(?,?,?,?,?,?,?) ON CONFLICT(network) DO UPDATE SET is_active=1,expires_at=NULL,updated_at=CURRENT_TIMESTAMP`).bind(crypto.randomUUID(),ip,`Trusted device: ${device.name}`,'Network approved through trusted-device settings','network',actor.role==='owner'?null:actor.id,actor.name).run();adminIpCache={expiresAt:0,entries:[]};await deviceHistory(env,device.id,actor.id,ip,'NETWORK_PERMANENTLY_AUTHORIZED','success')}await env.DB.prepare(`UPDATE admin_trusted_devices SET last_used_at=CURRENT_TIMESTAMP,last_ip=? WHERE id=?`).bind(ip,device.id).run();return ok(req,env,{granted:true,permanent:Boolean(body.permanent),deviceTrusted:true,redirect:'https://admin.shoplab.com.br/'},id)}async function createTrustedDevice(req,env,id,actor){const body=await readJson(req,8000),access=await requireSecurityOwner(req,env,actor,body);if(access.error)return access.error;const actorId=String(body.actorId||actor.id),actorType=actorId==='owner'?'owner':'collaborator';if(actorType==='collaborator'){const exists=await env.DB.prepare(`SELECT id FROM admin_collaborators WHERE id=? AND is_active=1`).bind(actorId).first();if(!exists)return fail(req,env,'ACTOR_NOT_FOUND','Active collaborator not found',404,id)}const token=bytesToHex(crypto.getRandomValues(new Uint8Array(32))),deviceId=crypto.randomUUID(),expiresAt=new Date(Date.now()+180*86400e3).toISOString();await env.DB.prepare(`INSERT INTO admin_trusted_devices(id,actor_id,actor_type,token_hash,name,description,browser_label,os_label,created_by,expires_at,last_ip) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(deviceId,actorId,actorType,await sha256(token),String(body.name||'Trusted device').slice(0,120),String(body.description||'').slice(0,500),String(body.browserLabel||'').slice(0,120),String(body.osLabel||'').slice(0,120),actor.name,expiresAt,trustedClientIp(req)).run();await deviceHistory(env,deviceId,actor.id,trustedClientIp(req),'TRUSTED_DEVICE_CREATED','success');return trustedDeviceCookie(ok(req,env,{id:deviceId,expiresAt},id),token,15552000)}
async function revokeTrustedDevice(req,env,id,actor,deviceId){const body=await readJson(req,4000),access=await requireSecurityOwner(req,env,actor,body);if(access.error)return access.error;if(body.action==='delete'){const found=await env.DB.prepare(`SELECT id FROM admin_trusted_devices WHERE id=?`).bind(deviceId).first();if(!found)return fail(req,env,'DEVICE_NOT_FOUND','Trusted device not found',404,id);await env.DB.batch([env.DB.prepare(`DELETE FROM admin_temporary_network_access WHERE device_id=?`).bind(deviceId),env.DB.prepare(`DELETE FROM admin_trusted_devices WHERE id=?`).bind(deviceId)]);await deviceHistory(env,deviceId,actor.id,trustedClientIp(req),'TRUSTED_DEVICE_DELETED','success');return ok(req,env,{deleted:true},id)}const result=await env.DB.prepare(`UPDATE admin_trusted_devices SET revoked_at=CURRENT_TIMESTAMP WHERE id=? AND revoked_at IS NULL`).bind(deviceId).run();if(!result.meta.changes)return fail(req,env,'DEVICE_NOT_FOUND','Trusted device not found',404,id);await env.DB.prepare(`UPDATE admin_temporary_network_access SET revoked_at=CURRENT_TIMESTAMP WHERE device_id=? AND revoked_at IS NULL`).bind(deviceId).run();await deviceHistory(env,deviceId,actor.id,trustedClientIp(req),'TRUSTED_DEVICE_REVOKED','success');return trustedDeviceCookie(ok(req,env,{revoked:true},id),'',0)}async function updateAdminSecuritySettings(req,env,id,actor){const body=await readJson(req,4000),access=await requireSecurityOwner(req,env,actor,body);if(access.error)return access.error;const mode=body.accessMode==='hybrid'?'hybrid':'strict';await env.DB.prepare(`UPDATE admin_security_settings SET access_mode=?,allow_collaborator_permanent_network=?,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id='global'`).bind(mode,body.allowCollaboratorPermanentNetwork?1:0,actor.name).run();await securityEvent(env,{action:'admin_security_policy_updated',actorId:actor.id,actorName:actor.name,sourceIp:trustedClientIp(req),result:'success',requestId:id,details:{mode}});return ok(req,env,{accessMode:mode},id)}
