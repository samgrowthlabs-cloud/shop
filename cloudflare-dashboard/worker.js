...
async function rankPremiumRelatedProductsWithAi(env, source, candidates) {
  ...
  const version = await sha256(`premium-related-v2|${source.slug}:${source.updatedAt}:${source.price}|${candidates.map((product) => `${product.slug}:${product.updatedAt}:${product.price}`).join("|")}`);
  try {
    ...
    const result = await env.AI.run(aiSetting.modelId, {
      ...
      gateway: {
        ...
        skipCache: false,
        cacheTtl: 2592000,
        cacheKey: `premium-related-v2:${version}`,
        collectLog: true,
...
async function adminAiProductDraft(req, env, id) {
  ...
  try {
    ...
    const result = await env.AI.run(aiSetting.modelId, {
      messages: [
        { role: "system", content: `Você auxilia o cadastro editorial de produtos da SHOPLAB. Gere português brasileiro claro, objetivo e sem exageros. Preserve marca, modelo, capacidade e medidas. Nunca invente especificações, avaliações, preços, garantias ou benefícios. Se um dado não estiver na entrada, omita-o. O slug deve usar somente a-z, 0-9 e hífen. categoryId deve ser exatamente um ID desta lista ou null: ${categoryList || "nenhuma categoria"}. productType deve ser book para livro físico, digital para produto digital e affiliate nos demais casos. A descrição curta deve ter até 300 caracteres. imageAlt deve descrever o produto de forma curta.` },
...
async function ensureShoplabAdsSchema(env){
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS shoplab_ads(id TEXT PRIMARY KEY,name TEXT NOT NULL,description TEXT,target_keywords TEXT NOT NULL DEFAULT '',public_title TEXT,ad_label TEXT NOT NULL DEFAULT 'PUBLICIDADE · SHOPLAB ADSENSE',show_header INTEGER NOT NULL DEFAULT 1,dismissible INTEGER NOT NULL DEFAULT 1,dismiss_minutes INTEGER NOT NULL DEFAULT 30,cta_text TEXT NOT NULL DEFAULT 'Saiba mais',cta_color TEXT NOT NULL DEFAULT '#075fce',old_price_text TEXT NOT NULL DEFAULT '',current_price_text TEXT NOT NULL DEFAULT '',old_price_color TEXT NOT NULL DEFAULT '#71807c',current_price_color TEXT NOT NULL DEFAULT '#087c70',media_type TEXT NOT NULL DEFAULT 'image',storage_key TEXT,link_url TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'draft',priority INTEGER NOT NULL DEFAULT 1,starts_at TEXT,ends_at TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`),
  ...
  const columns=[['target_keywords',`TEXT NOT NULL DEFAULT ''`],['public_title','TEXT'],['ad_label',`TEXT NOT NULL DEFAULT 'PUBLICIDADE · SHOPLAB ADSENSE'`],['show_header','INTEGER NOT NULL DEFAULT 1'],['dismissible','INTEGER NOT NULL DEFAULT 1'],['dismiss_minutes','INTEGER NOT NULL DEFAULT 30'],['cta_text',"TEXT NOT NULL DEFAULT 'Saiba mais'"],['cta_color',"TEXT NOT NULL DEFAULT '#075fce'"],['old_price_text',"TEXT NOT NULL DEFAULT ''"],['current_price_text',"TEXT NOT NULL DEFAULT ''"],['old_price_color',"TEXT NOT NULL DEFAULT '#71807c'"],['current_price_color',"TEXT NOT NULL DEFAULT '#087c70'"]];
...
async function adminShoplabAdAssignments(req,env,id){if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);const {results}=await env.DB.prepare(`SELECT id,ad_id adId,device,page_kind pageKind,position_key positionKey,category_slug categorySlug FROM shoplab_ad_assignments`).all();return ok(req,env,results||[],id)}
async function saveShoplabAdAssignments(req,env,id){if(!(await requireAdmin(req,env)))return fail(req,env,'UNAUTHORIZED','Não autorizado',401,id);await ensureShoplabAdsSchema(env);const body=await readJson(req,120000),items=Array.isArray(body.items)?body.items.slice(0,200):[],devices=new Set(['desktop','mobile']),pages=new Set(['home','products','category','product']);await env.DB.prepare(`DELETE FROM shoplab_ad_assignments`).run();let saved=0;for(const item of items){if(!devices.has(item.device)||!pages.has(item.pageKind)||!item.adId||!item.positionKey)continue;const adId=String(item.adId);await env.DB.batch([env.DB.prepare(`INSERT INTO shoplab_ad_assignments(id,ad_id,device,page_kind,position_key,category_slug) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),adId,item.device,item.pageKind,String(item.positionKey).slice(0,80),String(item.categorySlug||'').slice(0,100)),env.DB.prepare(`UPDATE shoplab_ads SET status='active',updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(adId)]);saved++}return ok(req,env,{saved},id)}
async function publicShoplabAds(req,env,id){
  ...
  const {results}=await env.DB.prepare(`SELECT a.id,a.name,a.target_keywords targetKeywords,a.public_title publicTitle,a.ad_label adLabel,a.show_header showHeader,a.dismissible,a.dismiss_minutes dismissMinutes,a.cta_text ctaText,a.cta_color ctaColor,a.old_price_text oldPriceText,a.current_price_text currentPriceText,a.old_price_color oldPriceColor,a.current_price_color currentPriceColor,a.media_type mediaType,a.storage_key storageKey,a.link_url linkUrl,a.priority,x.position_key positionKey FROM shoplab_ad_assignments x JOIN shoplab_ads a ON a.id=x.ad_id WHERE x.device=? AND x.page_kind=? AND (x.category_slug='' OR x.category_slug=?) AND a.status='active' AND (a.starts_at IS NULL OR datetime(a.starts_at)<=CURRENT_TIMESTAMP) AND (a.ends_at IS NULL OR datetime(a.ends_at)>=CURRENT_TIMESTAMP)`).bind(device,pageKind,category).all();
  let searches=[];
  ...
  const winners=[...new Map(ranked.map(row=>[row.positionKey,row])).values()];
  return ok(req,env,winners.map(({targetKeywords,priority,_matches,_random,...row})=>({...row,personalized:Boolean(_matches),mediaUrl:row.storageKey?`${origin}/media/${encodeURIComponent(row.storageKey)}`:null,storageKey:undefined})),id)
}
