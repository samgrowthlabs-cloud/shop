...
async function ensureShoplabAdsSchema(env){
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS shoplab_ads(id TEXT PRIMARY KEY,name TEXT NOT NULL,description TEXT,product_slug TEXT,target_keywords TEXT NOT NULL DEFAULT '',public_title TEXT,ad_label TEXT NOT NULL DEFAULT 'PUBLICIDADE · SHOPLAB ADSENSE',show_header INTEGER NOT NULL DEFAULT 1,dismissible INTEGER NOT NULL DEFAULT 1,dismiss_minutes INTEGER NOT NULL DEFAULT 30,cta_text TEXT NOT NULL DEFAULT 'Saiba mais',cta_color TEXT NOT NULL DEFAULT '#075fce',old_price_text TEXT NOT NULL DEFAULT '',current_price_text TEXT NOT NULL DEFAULT '',old_price_color TEXT NOT NULL DEFAULT '#71807c',current_price_color TEXT NOT NULL DEFAULT '#087c70',media_type TEXT NOT NULL DEFAULT 'image',storage_key TEXT,link_url TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'draft',priority INTEGER NOT NULL DEFAULT 1,starts_at TEXT,ends_at TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`),
  ...
  const columns=[['product_slug','TEXT'],['target_keywords',`TEXT NOT NULL DEFAULT ''`],['public_title','TEXT'],['ad_label',`TEXT NOT NULL DEFAULT 'PUBLICIDADE · SHOPLAB ADSENSE'`],['show_header','INTEGER NOT NULL DEFAULT 1'],['dismissible','INTEGER NOT NULL DEFAULT 1'],['dismiss_minutes','INTEGER NOT NULL DEFAULT 30'],['cta_text',"TEXT NOT NULL DEFAULT 'Saiba mais'"],['cta_color',"TEXT NOT NULL DEFAULT '#075fce'"],['old_price_text',"TEXT NOT NULL DEFAULT ''"],['current_price_text',"TEXT NOT NULL DEFAULT ''"],['old_price_color',"TEXT NOT NULL DEFAULT '#71807c'"],['current_price_color',"TEXT NOT NULL DEFAULT '#087c70'"]];
...
async function legacyPublicShoplabAds(req,env,id){
  ...
  const winners=[...new Map(ranked.map(row=>[row.positionKey,row])).values()];const adPrice=value=>value==null?'':'R$ '+(Number(value)/100).toFixed(2).replace('.',',');
...
async function publicShoplabAds(req,env,id){
  ...
  const winners=[...new Map(ranked.map(row=>[row.positionKey,row])).values()],adPrice=value=>value==null?'':'R$ '+(Number(value)/100).toFixed(2).replace('.',',');
  return ok(req,env,winners.map(({targetKeywords,priority,_matches,_random,...row})=>({...row,linkUrl:row.linkedProductSlug?'produto.html?slug='+encodeURIComponent(row.linkedProductSlug):row.linkUrl,oldPriceText:row.linkedProductSlug&&row.linkedOldPrice?adPrice(row.linkedOldPrice):row.oldPriceText,currentPriceText:row.linkedProductSlug&&row.linkedCurrentPrice?adPrice(row.linkedCurrentPrice):row.currentPriceText,personalized:Boolean(_matches),mediaUrl:row.storageKey?`${origin}/media/${encodeURIComponent(row.storageKey)}`:null,storageKey:undefined})),id)
}
